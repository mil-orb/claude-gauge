...
for profile in "$HOME/.zshrc" "$HOME/.bash_profile" "$HOME/.bashrc" "$HOME/.profile"; do
  if [[ -f "$profile" ]] && grep -q 'claude-gauge rate limit proxy' "$profile" 2>/dev/null; then
    if command -v sed &>/dev/null; then
      # Remove claude-gauge block using start/end markers (handles both legacy and conditional formats)
      if grep -q 'end claude-gauge' "$profile" 2>/dev/null; then
        sed -i.bak '/# claude-gauge rate limit proxy/,/end claude-gauge/d' "$profile"
      else
        # Legacy format without end marker: remove lines individually
